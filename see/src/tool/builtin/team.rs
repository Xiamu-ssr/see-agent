...
fn require_team_dir(ctx: &ToolContext) -> Result<&TeamDir> {
    ctx.team_dir.as_ref().ok_or_else(|| SeeError::Tool {
        tool: "team".to_owned(),
...
impl Tool for SendMessageTool {
    ...
    async fn execute(&self, args: Value) -> Result<ToolResult> {
        let to = args["to"].as_str().ok_or_else(|| SeeError::Tool {
            tool: "send_message".to_owned(),
            message: "missing 'to' parameter".to_owned(),
        })?;
        let content = args["content"].as_str().ok_or_else(|| SeeError::Tool {
            tool: "send_message".to_owned(),
...
impl Tool for ClaimTaskTool {
    ...
    async fn execute(&self, args: Value) -> Result<ToolResult> {
        ...
        let task_id = args["task_id"].as_str().ok_or_else(|| SeeError::Tool {
...
impl Tool for AssignTaskTool {
    ...
    async fn execute(&self, args: Value) -> Result<ToolResult> {
        ...
        let agent_id = args["agent_id"].as_str().ok_or_else(|| SeeError::Tool {
            tool: "assign_task".to_owned(),
