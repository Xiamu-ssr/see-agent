...
///
/// Each agent gets a dedicated worker process (`see-app worker <agent_id>`).
...
pub struct Supervisor {
    ...
    /// Path to the see-app binary. Defaults to the current executable.
    binary_path: PathBuf,
}

impl Supervisor {
    pub fn new(workspace: WorkspaceDir) -> Self {
        let binary_path = std::env::current_exe().unwrap_or_else(|_| PathBuf::from("see-app"));
        Self {
            workspace,
