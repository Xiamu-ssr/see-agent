...
impl Brain for OpenAiBrain {
    async fn chat(
        ...
        if !status.is_success() {
            ...
            return Err(SeeError::Brain {
                message: format!("API returned {status}: {text}"),
            });
        }

        let json: ApiResponse = resp.json().await.map_err(|e| SeeError::Brain {
...
fn parse_response(resp: ApiResponse) -> Result<BrainResponse> {
    let choice = resp.choices.into_iter().next().ok_or(SeeError::Brain {
