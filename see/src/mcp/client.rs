...
impl McpClient {
    ...
    pub async fn connect(&mut self) -> Result<()> {
        ...
        let child = cmd.spawn().map_err(|e| SeeError::Mcp {
    ...
    async fn send_request(&mut self, method: &str, params: Option<Value>) -> Result<Value> {
        let child = self.child.as_mut().ok_or_else(|| SeeError::Mcp {
        ...
        // Write to stdin
        let stdin = child.stdin.as_mut().ok_or_else(|| SeeError::Mcp {
            message: "MCP server stdin not available".into(),
        })?;
        stdin.write_all(&request_bytes).await.map_err(|e| SeeError::Mcp {
            message: format!("failed to write to MCP server '{}': {e}", self.name),
        })?;
        stdin.flush().await.map_err(|e| SeeError::Mcp {
            message: format!("failed to flush MCP server '{}': {e}", self.name),
        })?;

        // Read response from stdout
        let stdout = child.stdout.as_mut().ok_or_else(|| SeeError::Mcp {
        ...
        loop {
            line.clear();
            let bytes_read = reader.read_line(&mut line).await.map_err(|e| SeeError::Mcp {
                message: format!("failed to read from MCP server '{}': {e}", self.name),
            })?;

            if bytes_read == 0 {
                return Err(SeeError::Mcp {
    ...
    async fn send_notification(&mut self, method: &str, params: Option<Value>) -> Result<()> {
        ...
        stdin.write_all(&bytes).await.map_err(|e| SeeError::Mcp {
