use clap::Subcommand;
use see::types::{TeamMember, WorkspaceDir};
...
pub fn run(workspace: &WorkspaceDir, cmd: TeamCmd) {
    match cmd {
        TeamCmd::Create {
            ...
            match see::team::create_team(workspace, &name, parsed_members, leader.as_deref()) {
        ...
        TeamCmd::List => match see::team::list_teams(workspace) {
            Ok(teams) => {
                if teams.is_empty() {
        ...
        TeamCmd::Show { id } => match see::team::load_team(workspace, &id) {
            Ok(team) => {
        ...
        TeamCmd::Delete { id } => match see::team::delete_team(workspace, &id) {
