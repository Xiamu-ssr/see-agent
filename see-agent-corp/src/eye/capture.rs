...
impl Screenshot {
    ...
    pub fn save(&self, path: &std::path::Path) -> Result<()> {
        ...
        .map_err(|e| crate::error::CorpError::Agent {
...
impl Eye for MacEye {
    async fn capture(&self) -> Result<Screenshot> {
        ...
        let tmp_path = std::env::temp_dir().join("see-agent-corp-screenshot.png");
        ...
        if !output.status.success() {
            return Err(crate::error::CorpError::Agent {
