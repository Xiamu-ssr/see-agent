...
impl Screenshot {
    ...
    pub fn save(&self, path: &std::path::Path) -> Result<()> {
        ...
        .map_err(|e| crate::error::AgentCorpError::Agent {
...
impl Eye for MacEye {
    async fn capture(&self) -> Result<Screenshot> {
        ...
        let tmp_path = std::env::temp_dir().join("agentcorp-screenshot.png");
        ...
        let img = image::load_from_memory(&png_bytes).map_err(|e| {
            crate::error::AgentCorpError::Agent {
...
impl Eye for LinuxEye {
    async fn capture(&self) -> Result<Screenshot> {
        Err(crate::error::AgentCorpError::Agent {
