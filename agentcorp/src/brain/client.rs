...
impl Brain for OpenAiBrain {
    async fn chat(
        ...
        let resp = self
            ...
            .send()
            .await
            .map_err(|e| AgentCorpError::Brain {
        ...
        if !status.is_success() {
            ...
            return Err(AgentCorpError::Brain {
                message: format!("API returned {status}: {text}"),
            });
        }

        let json: ApiResponse = resp.json().await.map_err(|e| AgentCorpError::Brain {
...
fn parse_response(resp: ApiResponse) -> Result<BrainResponse> {
    let choice = resp.choices.into_iter().next().ok_or(AgentCorpError::Brain {
