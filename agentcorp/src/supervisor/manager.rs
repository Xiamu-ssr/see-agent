...
///
/// Each agent gets a dedicated worker process (`agentcorp worker <agent_id>`).
...
impl Supervisor {
    pub fn new(workspace: WorkspaceDir) -> Self {
        let binary_path = std::env::current_exe().unwrap_or_else(|_| PathBuf::from("agentcorp"));
        Self {
            workspace,
    ...
    ///
    /// The worker is launched as a child process: `agentcorp worker <agent_id>`
    pub async fn start_agent(&mut self, agent_id: &str) -> Result<()> {
        if self.processes.contains_key(agent_id) {
            return Err(AgentCorpError::Agent {
        ...
        if !agent_dir.path().exists() {
            return Err(AgentCorpError::NotFound {
        ...
        let child = tokio::process::Command::new(&self.binary_path)
            ...
            .spawn()
            .map_err(|e| AgentCorpError::Agent {
