...
/// Returns 0 if the cursor file doesn't exist.
pub fn read_cursor(path: &Path) -> Result<usize> {
    if !path.exists() {
        return Ok(0);
    }
...
mod tests {
    ...
    fn cursor_read_write() {
        ...
        assert_eq!(read_cursor(&path).unwrap(), 5);
    ...
    fn drain_reads_new_messages() {
        ...
        assert_eq!(read_cursor(&cursor).unwrap(), 3);
