...
///
///   DEFAULT_CONFIG < config.json < AGENTCORP_* env vars
///
...
pub fn load_config(workspace: &WorkspaceDir) -> Result<Config> {
    ...
    let config: Config = serde_json::from_value(merged).map_err(|e| AgentCorpError::Config {
...
///
///   DEFAULT_CONFIG < config.json < agent.json < AGENTCORP_* env vars
pub fn load_agent_config(workspace: &WorkspaceDir, agent_id: &str) -> Result<Config> {
    let global = {
        ...
        if workspace.config().exists() {
            ...
            let file_value: Value =
                serde_json::from_str(&file_content).map_err(|e| AgentCorpError::Config {
                    message: format!("invalid config.json: {e}"),
                })?;
            deep_merge(&default, &file_value)
        } else {
    ...
    let merged = if agent_dir.agent_json().exists() {
        ...
        let mut agent_value: Value =
            serde_json::from_str(&agent_content).map_err(|e| AgentCorpError::Config {
...
/// Apply AGENTCORP_* environment variable overrides.
///
...
fn apply_env_overrides(
    ...
    if let Ok(base_url) = get_env("AGENTCORP_BASE_URL") {
        set_nested(&mut value, &["llm", "base_url"], Value::String(base_url));
    }
    if let Ok(api_key) = get_env("AGENTCORP_API_KEY") {
        set_nested(&mut value, &["llm", "api_key"], Value::String(api_key));
    }
    if let Ok(model) = get_env("AGENTCORP_MODEL") {
...
mod tests {
    ...
    fn env_vars_override_all() {
        ...
        let env_map: HashMap<&str, String> = HashMap::from([
            ("AGENTCORP_MODEL", "env-model".to_owned()),
            ("AGENTCORP_API_KEY", "env-key".to_owned()),
            ("AGENTCORP_BASE_URL", "http://localhost:8080".to_owned()),
        ]);
        let get_env = |key: &str| {
            env_map
                .get(key)
    ...
    fn env_vars_partial_override() {
        ...
        let env_map: HashMap<&str, String> =
            HashMap::from([("AGENTCORP_MODEL", "custom-model".to_owned())]);
        let get_env = |key: &str| {
            env_map
                .get(key)
                .cloned()
