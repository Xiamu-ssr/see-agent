...
class TestSessionStore:
    ...
    def test_create_session(self, session_dir: Path) -> None:
        config = {"llm": {"model": "gpt-4o"}, "agent": {"max_steps": 50}}
        session = SessionStore.create("Open Safari", config, session_dir=session_dir)
    ...
    def test_load_session(self, session_dir: Path) -> None:
        created = SessionStore.create(
            "Test task", {"llm": {"model": "m"}, "agent": {"max_steps": 10}},
            session_dir=session_dir,
        )
        loaded = SessionStore.load(session_dir)
    ...
    def test_load_nonexistent_raises(self, tmp_path: Path) -> None:
        with pytest.raises(FileNotFoundError):
            SessionStore.load(tmp_path / "nonexistent")

    def test_delete_session(self, session_dir: Path) -> None:
        SessionStore.create("Delete me", {"llm": {"model": "m"}}, session_dir=session_dir)
        SessionStore.delete(session_dir)

        assert not session_dir.exists()

    def test_clean_session(self, session_dir: Path) -> None:
        SessionStore.create("Clean me", {"llm": {"model": "m"}}, session_dir=session_dir)
        deleted, freed = SessionStore.clean(session_dir)
        assert deleted == 1
        assert not session_dir.exists()
...
class TestSession:
    ...
    def test_append_and_read_messages(self, session_dir: Path) -> None:
        session = SessionStore.create("Test", {"llm": {"model": "m"}}, session_dir=session_dir)
    ...
    def test_update_meta(self, session_dir: Path) -> None:
        session = SessionStore.create("Test", {"llm": {"model": "m"}}, session_dir=session_dir)
    ...
    def test_screenshot_path(self, session_dir: Path) -> None:
        session = SessionStore.create("Test", {"llm": {"model": "m"}}, session_dir=session_dir)
    ...
    def test_next_step_number_empty(self, session_dir: Path) -> None:
        session = SessionStore.create("Test", {"llm": {"model": "m"}}, session_dir=session_dir)
...
class TestRestoreContext:
    ...
    def _make_session_with_history(
        ...
        session = SessionStore.create(
            "Search weather", {"llm": {"model": "m"}}, session_dir=session_dir,
        )
    ...
    def test_restore_context_has_history(
        ...
        session = self._make_session_with_history(session_dir)
    ...
    def test_restore_with_missing_screenshot(
        ...
        session = SessionStore.create(
            "Test", {"llm": {"model": "m"}}, session_dir=session_dir,
        )
...
class TestLogSystemPrompt:
    ...
    def test_log_system_prompt_writes_file(self, session_dir: Path) -> None:
        session = SessionStore.create("Test", {"llm": {"model": "m"}}, session_dir=session_dir)
    ...
    def test_log_system_prompt_skips_duplicate(self, session_dir: Path) -> None:
        session = SessionStore.create("Test", {"llm": {"model": "m"}}, session_dir=session_dir)
    ...
    def test_log_system_prompt_appends_on_change(self, session_dir: Path) -> None:
        session = SessionStore.create("Test", {"llm": {"model": "m"}}, session_dir=session_dir)
...
class TestSessionLogging:
    ...
    def test_setup_logging_creates_file(self, session_dir: Path) -> None:
        session = SessionStore.create("Test", {"llm": {"model": "m"}}, session_dir=session_dir)
    ...
    def test_teardown_logging_removes_handler(self, session_dir: Path) -> None:
        session = SessionStore.create("Test", {"llm": {"model": "m"}}, session_dir=session_dir)
        session.setup_logging()
...
class TestSessionMsgId:
    ...
    def test_append_message_includes_msg_id(self, session_dir: Path) -> None:
        session = SessionStore.create("Test", {"llm": {"model": "m"}}, session_dir=session_dir)
    ...
    def test_msg_counter_restored_on_read(self, session_dir: Path) -> None:
        session = SessionStore.create("Test", {"llm": {"model": "m"}}, session_dir=session_dir)
        session.append_message({"type": "user_task", "text": "a"})
        session.append_message({"type": "assistant", "content": "b"})
...
class TestSessionLoggingBug1:
    ...
    def test_teardown_restores_logger_levels(self, session_dir: Path) -> None:
        import logging as _logging
...
class TestRestoreCompact:
    ...
    def test_restore_handles_compact_entry(self, session_dir: Path) -> None:
...
class TestSessionEdgeCases:
    ...
    def test_append_message_unicode_roundtrip(
        ...
        session = SessionStore.create("测试", {"llm": {"model": "m"}}, session_dir=session_dir)
        session.append_message({"type": "user_task", "text": "打开Safari搜索🍓"})
        messages = session.read_messages()
        assert messages[0]["text"] == "打开Safari搜索🍓"

    def test_restore_context_empty_session(self, session_dir: Path) -> None:
        """Empty JSONL session restore_context returns context with system only."""
        session = SessionStore.create("empty", {"llm": {"model": "m"}}, session_dir=session_dir)
