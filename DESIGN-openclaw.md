...
> **核心原则**：一个 Agent 的所有持久数据（身份、配置、记忆、会话）都在 `agents/{id}/` 下。没有 `workspace/` 子目录。
...
```jsonc
{
    ...
    // ── 沙箱配置（可选）──
    "sandbox": {
        "profile": "default",           // sandbox-exec profile 名
...
`build_system_prompt()` 按以下顺序读取并拼接到 system prompt：
...
**文件规则**：只允许 `MEMORY.md` 和 `YYYY-MM-DD.md` 两种文件名，其他会被 `write_memory` tool 拒绝。

**MEMORY.md — 长期记忆**：
- Agent 跨 session 的精华知识库
- 自动注入到 system prompt（见 2.2 第 6 项）
- Agent 通过 `write_memory` tool 主动写入
- 内容应该是精炼的、有价值的——不是流水账

**YYYY-MM-DD.md — 日记**：
- 按天记录的原始笔记，不注入 system prompt
- Agent 通过 `memory_search` tool 搜索历史日记
- 写入时机：Agent 主动调用 `write_memory`（自觉）+ context compact 时自动将 summary 写入当天日记

**搜索机制**：BM25 关键词搜索，对 `memory/` 下所有 `*.md` 文件的段落建索引。中文字符 bigram 分词，英文空格分词。
...
#### 2.4.2 messages.jsonl

每行一个 JSON 对象，公共字段：`msg_id`（自增）、`ts`（ISO-8601）、`type`。

这是完整的不可变对话流水账。compact 不删除旧行，只追加 `type=compact` 标记。
...
| `user_task` | 用户任务（含截图引用） | `text`, `screenshot`, `detail` |
| `assistant` | LLM 回复 | `content`, `tool_calls[{id, name, args}]` |
| `tool_result` | 工具执行结果 | `tool_call_id`, `result` |
| `screenshot` | 独立截图消息 | `screenshot`, `detail` |
| `user_reply` | 用户中途回复 | `text` |
| `system_hint` | 系统提示（无进展警告等） | `text` |
| `compact` | 压缩标记 | `summary`, `first_kept_msg_id` |

#### 2.4.3 Context Compact 机制

**触发**：`compact.enabled=true` 且估算 token > `context_window × target_ratio`

**流程**：
1. 取 messages[1:-keep_recent]（跳过 system，保留最近 N 条）
2. 调 `brain.summarize()` 生成摘要
3. 替换上下文中的旧消息为摘要
4. 向 messages.jsonl 追加 `type=compact` 记录
5. 同时触发日记写入 → `memory/YYYY-MM-DD.md`

**compact 后下次推理携带**：system prompt（完整）+ compact summary（作为 `[Conversation Summary]`）+ 最近 `keep_recent` 条原始消息

**恢复 session**：读 messages.jsonl 找最后一条 `type=compact` → 跳过 `msg_id < first_kept_msg_id` → 用 summary + 保留消息重建上下文

---

## 三、teams/ — Team 数据目录

Team 是轻量「房间」——成员列表 + leader + 通信记录。Agent 数据不在 team 目录下。
...
| `status` | string | ❌ | `"created"` | 状态：created / running / stopped |
| `created_at` | string | ❌ | — | ISO-8601 创建时间 |

### 3.2 messages.jsonl

Team 内所有跨 Agent 通信和用户消息，每行一个 JSON：

```jsonc
{ "sender": "owner", "recipient": "xxxx4", "content": "你好", "ts": "2026-03-11T10:28:24+00:00" }
```
...
- 内置 skill 在 `see_agent/builtin_skills/`，首次启动自动复制到此处（不覆盖已有）
- ClawHub 安装的 skill 也放这里
- 搜索路径由 `config.json` 的 `skills_dirs` 配置
...
- 按天命名 `YYYY-MM-DD.log`，`RotatingFileHandler` 10MB/5 backups
- 格式：`HH:MM:SS  LEVEL     logger  message`
- 全局日志只记 WARNING 级别的 agent/brain/eye/hand
...
纯运行时数据，进程重启后可全部重建，不含持久化数据。

```
run/agents/{agent_id}/
├── config.json      # Supervisor 写给 Worker 的合并后配置
├── inbox.jsonl      # 消息收件箱（过渡方案，UDS push 完善后废弃）
└── agent.sock       # UDS socket
```

### 6.1 run config.json 中的内部字段

Supervisor 合并配置后额外注入的 `_` 前缀字段：

| 字段 | 含义 |
|------|------|
| `_agent_id` | Agent ID |
| `_session_root` | → `agents/{id}/sessions` |
| `_memory_dir` | → `agents/{id}/memory` |
| `_leader_id` | Team leader ID |
| `_denied_tools` | 禁用工具列表（来自 `tools_config.disabled`） |
| `_team_context` | Team 上下文字符串 |
| `_owner_display` | 拥有者显示名 |
| `_result_path` | Worker 写结果的路径 |
...
位置：`~/.see-agent/config.json`，最低优先级，可被 team.json 和 agent.json 覆盖。
...
| `language` | string | `zh` | 语言（zh/en），影响 system prompt |
| `max_steps` | int | `50` | 单 session 最大步数 |
| `max_images` | int | `5` | 上下文滑动窗口保留截图数 |
| `screenshot_interval_ms` | int | `800` | 截图间隔 ms |
| `tool_delay_ms` | int | `200` | 工具执行间隔 ms |
| `scaling_enabled` | bool | `true` | 屏幕缩放开关 |
| `scaling_match` | string | `aspect_ratio` | 缩放策略：aspect_ratio / pixel_count |
| `context_engine` | string | `default` | 上下文引擎 |
| `skills_dirs` | string[] | `["~/.see-agent/skills"]` | Skill 搜索路径 |
| `memory.enabled` | bool | `true` | 记忆开关 |
| `memory.search.mode` | string | `bm25` | 搜索模式 |
| `compact.enabled` | bool | `false` | 自动压缩开关 |
| `compact.context_window` | int | `128000` | 模型上下文窗口（tokens） |
| `compact.target_ratio` | float | `0.75` | 压缩阈值（token > window*ratio 触发） |
| `compact.keep_recent` | int | `8` | 压缩后保留最近 N 条消息 |
| `compact.summary_model` | string | `""` | 摘要模型（空=用主模型） |
| `env` | object | `{}` | 注入 Agent 进程的环境变量 |
| `mcp_servers` | object | `{}` | MCP Server 配置（见下） |
| `show_overlay` | bool | `true` | PyObjC 透明覆盖层开关 |

**mcp_servers 中每个 server 的字段**：

```jsonc
{
    "type": "stdio",           // stdio | sse
    "command": "npx",          // 启动命令
    "args": ["tavily-mcp"],    // 参数列表
    "env": {                   // 该 server 独有的环境变量
        "API_KEY": "xxx"
    }
}
```
...
config.json（全局默认）
  ↓ deep merge
team.json.config_overrides（Team 级别，预留未实现）
  ↓ deep merge
agent.json.config_overrides（Agent 级别）
  ↓ 最高优先级
