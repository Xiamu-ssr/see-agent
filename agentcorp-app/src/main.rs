...
use agentcorp::config::{ensure_workspace, load_config};
use agentcorp::consts::VERSION;
use agentcorp::types::WorkspaceDir;

fn main() {
    let cli = Cli::parse();
    let workspace = WorkspaceDir::new(agentcorp::config::resolve_workspace_root());
    ...
    match cli.command {
        ...
        Commands::Status => {
            ...
            println!("agentcorp v{VERSION}");
            println!("Workspace: {}", workspace.path().display());
            println!("LLM model: {}", config.llm.model);

            let agents = agentcorp::agent::list_agents(&workspace).unwrap_or_default();
            println!("Agents: {}", agents.len());

            let teams = agentcorp::team::list_teams(&workspace).unwrap_or_default();
...
fn create_system_agents(workspace: &WorkspaceDir) {
    ...
    if !system_dir.path().exists() {
        let _ = agentcorp::agent::create_agent(workspace, "system", Some("System"), Some("🔧"));
    }
}
