...
use agentcorp::agent::{AgentLoop, ConversationContext};
use agentcorp::brain::{build_system_prompt, OpenAiBrain, PromptContext};
use agentcorp::config::load_agent_config;
use agentcorp::tool::{register_builtin_tools, ToolContext, ToolRegistry};
use agentcorp::types::WorkspaceDir;

/// Run as a worker process for a single agent.
///
/// This is not invoked by users directly — the supervisor spawns it:
/// `agentcorp worker <agent_id> <workspace_path>`
pub async fn run(agent_id: &str, workspace_path: &str) {
    ...
    let eye: Arc<dyn agentcorp::eye::Eye> = create_eye();
    ...
    let (wake_tx, mut wake_rx) =
        tokio::sync::mpsc::channel::<()>(agentcorp::consts::WORKER_WAKE_CHANNEL_SIZE);
    ...
    loop {
        ...
        if let Ok((steer_msgs, collect_msgs)) =
            agentcorp::supervisor::drain_inbox_split(&inbox_path, &cursor_path)
        {
            // Check for shutdown
        ...
        let timeout =
            tokio::time::Duration::from_secs(agentcorp::consts::WORKER_SIGNAL_TIMEOUT_SECS);
...
#[cfg(target_os = "macos")]
fn create_eye() -> Arc<dyn agentcorp::eye::Eye> {
    Arc::new(agentcorp::eye::MacEye::new())
}

#[cfg(target_os = "linux")]
fn create_eye() -> Arc<dyn agentcorp::eye::Eye> {
    Arc::new(agentcorp::eye::LinuxEye)
}
...
fn find_agent_team(
    ...
    agent_id: &str,
) -> Option<agentcorp::types::paths::TeamDir> {
