use clap::Subcommand;
use agentcorp::types::{TeamMember, WorkspaceDir};
...
pub fn run(workspace: &WorkspaceDir, cmd: TeamCmd) {
    match cmd {
        TeamCmd::Create {
            ...
            match agentcorp::team::create_team(workspace, &name, parsed_members, leader.as_deref()) {
        ...
        TeamCmd::List => match agentcorp::team::list_teams(workspace) {
            Ok(teams) => {
                if teams.is_empty() {
        ...
        TeamCmd::Show { id } => match agentcorp::team::load_team(workspace, &id) {
            Ok(team) => {
        ...
        TeamCmd::Delete { id } => match agentcorp::team::delete_team(workspace, &id) {
