...
pub fn run(workspace: &WorkspaceDir, cmd: AgentCmd) {
    match cmd {
        AgentCmd::Create { id, name, emoji } => {
            ...
            match agentcorp::agent::create_agent(
                workspace,
                &agent_id,
                name.as_deref(),
                emoji.as_deref(),
        ...
        AgentCmd::List => match agentcorp::agent::list_agents(workspace) {
            Ok(agents) => {
                if agents.is_empty() {
        ...
        AgentCmd::Show { id } => match agentcorp::agent::load_agent(workspace, &id) {
            Ok(def) => {
        ...
        AgentCmd::Delete { id } => match agentcorp::agent::delete_agent(workspace, &id) {
