...
async fn update_config_handler(
    ...
    if let Some(obj) = updates.as_object() {
        ...
        let merged = agentcorp::config::deep_merge(&current, &Value::Object(obj.clone()));
        ...
        // Reload config
        let new_config =
            agentcorp::config::load_config(ws).map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
